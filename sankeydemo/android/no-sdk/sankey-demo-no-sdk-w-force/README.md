# Sankey Demo (No SDK, with Swipe-Up Force Quit)

An Android demo app that simulates e-commerce user journeys with **swipe-up force quit behavior** that produces `USER_REQUESTED` app exit reasons. Uses a companion "Killer" app with an Accessibility Service to simulate a user swiping away the app from the recent apps screen.
...
- **Logcat logging** for all navigation and events (no SDK)
...
6. Demo app restarts and automatically begins simulation again (infinite loop)

**Note:** The 1.5 second delay before restart is important - it allows Android to fully terminate the process and record `ApplicationExitInfo`.
