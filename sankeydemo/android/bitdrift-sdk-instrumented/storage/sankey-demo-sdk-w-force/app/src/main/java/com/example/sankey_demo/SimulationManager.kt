...
class SimulationManager : ViewModel() {
     ...
     * Steps 1-4: low probability (3%), Steps 5-7: high probability (10%)
     */
    private suspend fun runSingleJourney(navController: NavController) {
        // Step 1: Reset to Welcome (low quit probability - 3%)
        navController.navigate(Screen.Welcome.route) {
            popUpTo(Screen.Welcome.route) { inclusive = true }
        }
        delay(stepDelay)
        if (AppRestartManager.maybeForceQuit(Screen.Welcome.route, step = 1)) return
